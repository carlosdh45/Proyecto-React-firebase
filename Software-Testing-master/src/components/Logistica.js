...
import LogisticaFormulario from "./LogisticaFormulario";
...
useEffect(()=>{
  firebaseDb.child('tareas/logistica').on('value',snapshot =>{
  ...
  const addOrEdit = obj=>{
    if(currentId=='')
firebaseDb.child('tareas/logistica').push(
  obj,
  err =>{
    if(err)
    console.log(err)
...
firebaseDb.child(`tareas/logistica/${currentId}`).set(
  obj,
  err =>{
    if(err)
    console.log(err)
  ...
  <div className="container">
    <h1 className="display-4 text-center">Registrar Tareas de Logistica</h1>
  </div>
</div>
 <div className="row">
 <div className="col-md-5">
     <LogisticaFormulario {...({addOrEdit, currentId, tareaObjects})}/>
 </div>
